...
parasails.registerComponent('infoCard', {
    //  ╔═╗╦═╗╔═╗╔═╗╔═╗
    //  ╠═╝╠╦╝║ ║╠═╝╚═╗
    //  ╩  ╩╚═╚═╝╩  ╚═╝
    props: [
    ...
    //  ╩ ╩ ╩ ╩ ╩╩═╝
    template: `
    <div class="card w-28 bg-info">
      <div class="card-body" style="width: 20rem; height: 200px;">
        <h3>{{title}}</h3>
        <hr></hr>
        <p class ="card-text">{{paragraph}}</p>
        <button :onclick="url"><a :href="url">{{button}}</a></button>
      </div>
    </div>
    `,
    //  ╦  ╦╔═╗╔═╗╔═╗╦ ╦╔═╗╦  ╔═╗
